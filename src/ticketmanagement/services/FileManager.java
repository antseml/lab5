...
/**
 * Загрузка и сохранение коллекции в JSON (как в предыдущей версии лабораторной).
 */
public class FileManager {
    ...
    private Person parsePerson(String personStr) {
        try {
            ...
            if (personStr.startsWith("{")) {
                personStr = personStr.substring(1);
            }
            ...
            for (String pair : pairs) {
                ...
                switch (k) {
                    ...
                    case "eyeColor":
                        ...
                        break;
                    case "hairColor":
                        hairColor = parseHairColor(v);
                        break;
                    case "nationality":
                        nationality = Country.valueOf(v);
                        break;
                    default:
                        break;
                }
    ...
    private static EyeColor parseEyeColor(String raw) {
        try {
            return EyeColor.valueOf(raw);
        } catch (IllegalArgumentException ignored) {
            return legacyToEyeColor(raw);
        }
    }

    private static HairColor parseHairColor(String raw) {
        try {
            return HairColor.valueOf(raw);
        } catch (IllegalArgumentException ignored) {
            return legacyToHairColor(raw);
        }
    }

    /** Совместимость со старым единым enum Color из прежней версии. */
    ...
    private static HairColor legacyToHairColor(String old) {
        switch (old) {
            case "YELLOW":
